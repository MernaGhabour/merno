...
module.exports.getProduct = function(req, res, next) {
  ...
  Merna.findById(req.params.productId).exec(function(err, Merna) {
    if (err) {
      return next(err);
    }
    if (!Merna) {
...
module.exports.getProducts = function(req, res, next) {
  Merna.find({}).exec(function(err, Merna) {
    if (err) {
      return next(err);
    }
    res.status(200).json({
...
module.exports.createProduct = function(req, res, next) {
  ...
  Merna.create(req.body, function(err, Merna) {
    if (err) {
      return next(err);
    }
    res.status(201).json({
...
module.exports.deleteProduct = function(req, res, next) {
  ...
  Merna.findByIdAndRemove(req.params.productId).exec(function(
    err,
    deletedProduct
  ) {
    if (err) {
